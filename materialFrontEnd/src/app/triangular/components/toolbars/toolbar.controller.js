(function() {
    ...
    /* @ngInject */
    function DefaultToolbarController($scope, $rootScope, $mdMedia, $http, $translate, $state, $element, $filter, $mdUtil, $mdSidenav, Auth, $mdToast, $timeout, $document, triBreadcrumbsService, triSettings, triLayout) {
        var vm = this;
        ...
        $scope.ifRoomEditAllowed = false;

        $http({
            url: 'http://localhost:3000/api/haspermission',
            method: "POST",
            data: $.param({token:Auth.getToken(), permission: 'edit_room'}),
            headers: {'Content-Type': 'application/x-www-form-urlencoded'}
        }).success(function(data){
            $scope.ifRoomEditAllowed = data.data;
        }).error(function(err){
            $scope.ifRoomEditAllowed = true;
        });
        // initToolbar();
        $http({
            ...
            $scope.user_name = data.info.name;
        }).error(function(err){
            console.log(err);
        });
        ////////////////
